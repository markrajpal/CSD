...
public class TestPhaserCharacterizationTests {
    private Game game;
    ...
    public void gameHasFireWeaponSignatureExpectedBySampleClient() {
		...
		assertTrue("The following signature must exist else clients will not compile: " + expectedSignature, found);
    }
    ...
    public void PhasersFiredWithInsufficientEnergy() {
        ...
        assertEquals("Insufficient energy to fire phasers! || ",
    ...
    public void PhasersFiredWhenKlingonOutOfRange_AndEnergyExpendedAnyway() {
        ...
        game.fireWeapon(context);
        assertEquals("Klingon out of range of phasers at " + outOfRange + " sectors... || ",
                context.getAllOutput());
        assertEquals(EnergyInNewGame - 1000, game.EnergyRemaining());
    }
    ...
    public void PhasersFiredKlingonDestroyed() {
        ...
        game.fireWeapon(context);
        assertEquals("Phasers hit Klingon at 2000 sectors with 400 units || Klingon destroyed! || ",
                context.getAllOutput());
        assertEquals(EnergyInNewGame - 1000, game.EnergyRemaining());
        assertTrue(klingon.deleteWasCalled());
    }
    ...
    public void PhasersDamageOfZeroStillHits_AndNondestructivePhaserDamageDisplaysRemaining() {
        ...
        Game.generator = new MockRandom();
        game.fireWeapon(context);
        assertEquals("Phasers hit Klingon at 2000 sectors with " +
